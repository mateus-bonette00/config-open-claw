...
cd /home/bonette/openclaw-agents && bash scripts/run-fba-from-html.sh --mode resume
```
...
Comandos:
```bash
# No notebook (onde esta o repo):
cd /home/mateus/Documentos/Projetos/config-open-claw && npm run deploy:live

# Checar gateway no servidor:
ssh openclaw-server 'systemctl --user is-active openclaw-gateway'

# Criar tunnel no notebook:
cd /home/mateus/Documentos/Projetos/config-open-claw && bash scripts/ssh-tunnel.sh
```
