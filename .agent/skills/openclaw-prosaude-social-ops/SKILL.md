...
1. `agents/prosaude-social/index.js`
2. `.env`
3. `storage/logs/prosaude-social.log`
4. `storage/state/prosaude-social.json`
5. `scripts/sync-prosaude-artes.sh`

## Guia rapido (passo a passo)
1. Confirme a imagem de entrada e o nome do produto.
2. Abra `.env` e confira `REMOVEBG_API_KEY` (se nao tiver `rembg` local).
3. Abra `.env` e confira `META_ACCESS_TOKEN`, `META_PAGE_ID`, `INSTAGRAM_ACCOUNT_ID`.
4. Veja o log em `storage/logs/prosaude-social.log`.
5. Se quiser apenas validar, rode o fluxo com `approve=true` (nao publica).
6. Se for publicar, confirme URL publica da imagem (nao usar caminho local).
7. Execute o teste minimo e valide o output em `storage/prosaude-output`.
...
1. `node agents/prosaude-social/index.js`
2. `bash scripts/sync-prosaude-artes.sh`
